...
UNMAP_AFTER_INIT void StorageManagement::enumerate_controllers(bool force_pio, bool poll)
{
        ...
        PCI::enumerate([&](PCI::DeviceIdentifier const& device_identifier) {
...

            {
                ...
                if (device_identifier.hardware_id() == vmd_device) {
                    ...
                    PCI::Access::the().add_host_controller_and_enumerate_attached_devices(move(controller), [this, poll](PCI::DeviceIdentifier const& device_identifier) -> void {
                        auto subclass_code = static_cast<SubclassID>(device_identifier.subclass_code().value());
                        if (subclass_code == SubclassID::NVMeController) {
                            auto controller = NVMeController::try_initialize(device_identifier, poll);
            ...
            if (subclass_code == SubclassID::NVMeController) {
                dbgln("NVMecontroller polled mode{}", kernel_command_line().is_storage_polling_enabled());
                auto controller = NVMeController::try_initialize(device_identifier, poll);
                if (controller.is_error()) {
                    dmesgln("Unable to initialize NVMe controller: {}", controller.error());
                } else {
